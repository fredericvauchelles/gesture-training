...
use crate::app::{AppWorkflow, Message, State};
use crate::col;
...
impl AppWorkflow for WorkflowPrepareSession {
    ...
    fn update(state: &mut State, message: Self::WorkflowMessage) -> Command<Self::AppMessage> {
        match message {
            ...
            MessagePrepareSession::StartSession => Command::perform(
                WorkflowRunSession::new(
                    state.session_configuration.clone(),
                    state.session_prepared.clone(),
                ),
            ...
            MessagePrepareSession::SetImageTime(value) => {
                state.session_configuration.image_time =
                    ImageTime::FixedTime(Duration::from_secs(value as u64));
                Command::none()
            }
            MessagePrepareSession::SelectImageFolder => {
                let future = async { AsyncFileDialog::new().pick_folder().await };
                Command::perform(future, |file_handle| {
                    Message::PrepareSession(MessagePrepareSession::SetImageFolder(
                        file_handle.map(|handle| handle.path().to_path_buf()),
                    ))
                })
            }
            MessagePrepareSession::SetImageFolder(option_path) => {
                state.session_configuration.image_selection.folder_path =
                    option_path.unwrap_or(PathBuf::new());

                Command::perform(
                    Self::compute_new_prepared_session(state.session_configuration.clone()),
                    |prepared_session| match prepared_session {
                        Ok(prepared_session) => Message::PrepareSession(
    ...
    fn view(&self, state: &State) -> Element<Self::AppMessage> {
        ...
        let text_folder_input_label = text("Image folder").width(Length::Fixed(150.0));
        let text_folder_selected = text(
            state
        ...
        .width(Length::Fill);
        let button_choose_folder = button("Pick").on_press(Message::PrepareSession(
            MessagePrepareSession::SelectImageFolder,
        ));
        let row_folder_selector = row!(
            text_folder_input_label,
            text_folder_selected,
        ...
        let text_image_counts_label = text("Image Counts").width(Length::Fixed(150.0));
        let radio_image_counts_items = (1..5u8)
            .map(|i| {
                let amount = i * 5;
                radio(
                    amount.to_string(),
                    amount,
                    Some(state.session_configuration.image_count),
                    |_value| -> Message { MessagePrepareSession::SetImageCount(amount).into() },
                )
            })
            .map(Element::from);
        let radio_image_counts = Row::with_children(
            [Element::from(text_image_counts_label)]
                .into_iter()
                .chain(radio_image_counts_items),
        )
        .align_items(Alignment::Start)
        .spacing(5)
        .width(Length::Fill);

        let text_duration_label = text("Duration").width(Length::Fixed(150.0));
        let radio_duration_items = ([5, 30, 60, 90, 120, 240, 600, 1200])
            .map(|i| {
                let amount = i;
                ...
                radio(amount.to_string(), amount, selected, |_value| -> Message {
                    MessagePrepareSession::SetImageTime(amount).into()
                })
            })
            .map(Element::from);
        let radio_duration = Row::with_children(
            [Element::from(text_duration_label)]
                .into_iter()
                .chain(radio_duration_items),
        )
        .align_items(Alignment::Start)
        .spacing(5)
        .width(Length::Fill);
        ...
        let button_start = button(
            text(format!(
                "Start ({} images)",
                state.session_prepared.valid_images.len()
            ))
            .width(Length::Fill)
            .horizontal_alignment(Horizontal::Center),
        )
        ...
        col!(text_title, space, control_panel).padding(20).into()
    }
...
impl WorkflowPrepareSession {
    ...
    async fn compute_new_prepared_session(
        session_configuration: SessionConfiguration,
    ) -> io::Result<StatePreparedSession> {
        let valid_images =
            Self::find_image_files_in_directory(&session_configuration.image_selection.folder_path)
                .await?;
        Ok(StatePreparedSession { valid_images })
    }
}
...
mod test {
    ...
    fn test_image_extension() {
        assert!(WorkflowPrepareSession::is_image_file(&Path::new(
            &"test.jpg"
        )));
    }
}
